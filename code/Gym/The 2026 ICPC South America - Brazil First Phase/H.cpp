...
// URL: https://codeforces.com/gym/106679/problem/H
...
void solve() {
    int n, m;
    cin >> n >> m;
    vector<int> lights(n, 0), d(n + 1, 0);
    for (auto& i : lights) cin >> i;
    d[0] = lights[0], d[n] = lights[n - 1];
    for (int i = 1; i < n; i++) {
        d[i] = lights[i-1] ^ lights[i];
    }
