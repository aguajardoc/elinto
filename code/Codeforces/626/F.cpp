    ...
    for (int i = 1; i <= n; i++) {
    	...
    	for (int j = 0; j <= n; j++) {
    		for (int k = 0; k <= x; k++) {
    			...
    			// Add to new team, close
    			dp[now][j][nb] += dp[last][j][k];
    			dp[now][j][nb] %= MOD;
    			
    			// Add to new team, dont close
    			if (j < n) {
    				dp[now][j+1][nb] += dp[last][j][k];
    				dp[now][j+1][nb] %= MOD;
    			}
    			
    			// Add to existing team, close
    			if (j) {
    				dp[now][j-1][nb] += j*dp[last][j][k];
    				dp[now][j-1][nb] %= MOD;
    			}
    			
    			// Add to existing team, dont close
    			dp[now][j][nb] += j*dp[last][j][k];
    			dp[now][j][nb] %= MOD;
    		}
    	}
    	
    	fill_n(&dp[last][0][0], 201*1001, 0);
    }
